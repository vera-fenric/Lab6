    ...
    public class V2DataOnGrid : V2Data, IEnumerable<DataItem>
    {
        ...
        public double Min
        {
            get{
                if (array == null)
                    return 0;
                if (Grid[0].Nodes == 0 || Grid[1].Nodes == 0)
                    return 0;
                double min;
                min = Complex.Abs(array[0, 0]);
                for (int i=0; i<Grid[0].Nodes; i++)
                    for (int j = 0; j < Grid[1].Nodes; j++)
                    {
                        if (min > Complex.Abs(array[i, j]))
                            min = Complex.Abs(array[i, j]);
                    }
                return min;
            }
        ...
        public double Max
        {
            get
            {
                ...
                if (Grid[0].Nodes == 0 || Grid[1].Nodes == 0)
                    return 0;
                max = Complex.Abs(array[0, 0]);
                for (int i = 0; i < Grid[0].Nodes; i++)
                    for (int j = 0; j < Grid[1].Nodes; j++)
                    {
                        if (max < Complex.Abs(array[i, j]))
                            max = Complex.Abs(array[i, j]);
                    }
                return max;
            }
        }
